...
const GallerySection = () => {
  return (
    <section className="relative bg-gradient-to-b from-white via-gray-50 to-gray-100 py-24 overflow-hidden" id="gallery">
      {/* Decorative Glow Gradient */}
      <div className="absolute -top-20 -left-20 w-[500px] h-[500px] bg-red-600/30 rounded-full blur-[150px]"></div>
      <div className="absolute -bottom-20 -right-20 w-[500px] h-[500px] bg-red-500/20 rounded-full blur-[150px]"></div>
        ...
        <div className="relative">
          ...
          >
            {galleryItems.map((item, idx) => (
              <SwiperSlide key={idx}>
                <div className="relative group overflow-hidden rounded-2xl shadow-2xl hover:shadow-red-200 transition-shadow duration-500">
                  {/* Image */}
                  <img
                    src={item.img}
                    alt={item.caption}
                  ...
                  <div className="absolute bottom-0 left-0 right-0 text-center mb-6">
                    <span className="inline-block bg-red-600/90 backdrop-blur-sm text-white text-base font-semibold px-5 py-2 rounded-full shadow-lg">
                      {item.caption}
                    </span>
                  </div>
                </div>
          ...
          <div className="absolute top-1/2 -translate-y-1/2 left-0 right-0 flex justify-between px-3 md:px-4 z-20">
            <button className="swiper-prev bg-white/90 hover:bg-red-600 text-red-600 hover:text-white rounded-full p-3 md:p-4 shadow-lg transition-all duration-300 hover:shadow-red-400/50 border border-red-600">
              <FaChevronLeft className="text-xl" />
            </button>
            <button className="swiper-next bg-white/90 hover:bg-red-600 text-red-600 hover:text-white rounded-full p-3 md:p-4 shadow-lg transition-all duration-300 hover:shadow-red-400/50 border border-red-600">
